    ...
    const loggerWithMetadata = new Proxy(unlockedLogger, {
        get(target, prop) {
            ...
            if (!shouldSkipMethod) {
                return (...args) => {
                    const [message, metadata] = args;
                    target[prop](message, merge__default["default"](metadata, metadataGetter(metadata)));
                };
            }
            return target[prop];
        },
 ...
 * logger.
 */
const fallbackLogger = injectMetadata(lockLogger(logger.LoggerFactory.create(logger.LoggerType.ConsolaGcp)), () => ({
    alokai: {
        context: "middleware",
    },
}));
...
const createReadyzHandler = (readinessChecks) => async () => {
    ...
    if (readinessCheckFailureReasons.length) {
        throw new terminus.HealthCheckError("Readiness check failed", readinessCheckFailureReasons);
    }
};
...
async function createServer(config, options = {}) {
    const loggerManager = new LoggerManager(config, (loggerConfig) => lockLogger(logger.LoggerFactory.create(logger.LoggerType.ConsolaGcp, loggerConfig)));
...
exports.createReadyzHandler = createReadyzHandler;
